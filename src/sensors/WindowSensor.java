...
public class WindowSensor extends Sensor implements Runnable {
    ...
    private float currentWindowState = 0;
...
//of a percentage of the screen height
            ...
            messageWin.writeMessage("   Initial Window Set:: " + currentWindowState);
/**
 * ******************************************************************
...
// Post the current temperature
                postEvent(evtMgrI, WINDOW, currentWindowState);
                messageWin.writeMessage("Current Window state is::  " + currentWindowState + " .");
...
// output of the temperature as it would in reality.
                ...
                for (int i = 0; i < qlen; i++) {
                    evt = queue.getEvent();
                    if (evt.getEventId() == WINDOW_SENSOR) {
                        if (evt.getMessage().equalsIgnoreCase(WINDOW_ON)) // heater on
                        {
                            currentWindowState = 0;
                        } // if
                        if (evt.getMessage().equalsIgnoreCase(WINDOW_OFF)) // heater off
                        {
                            currentWindowState = 1;
                        } // if
                    } // if
